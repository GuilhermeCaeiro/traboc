...
function main(args)
    ...
    strategy = args[1]
    if strategy == "-help"
        show_info( "jgvtspsolver.jl <christofides|gurobi> <testdatafile> <max_iterations> <gap_threshold> <epsilon> <current|best|5pct|1pct> <static|lbdecay|itdecay> <epsilon_decay_interval> <epsilon_decay_multiplier> [<check_point>]" )
        ...
        show_info( "<static|lbdecay1|lbdecay2|itdecay> strategy to update epsilon" )
    ...
    epsilon_decay_interval = parse(Int64, args[8])
    epsilon_decay_multiplier = parse(Float64,args[9])
    check_point = max_iterations
    if length(args) > 9
        check_point = parse(Int64,args[10])
    end
    ...
    show_info("epsilon_strategy: ", epsilon_strategy)
    show_info("epsilon_decay_interval: ", epsilon_decay_interval)
    show_info("epsilon_decay_multiplier: ", epsilon_decay_multiplier)
    ...
    if strategy in ["christofides", "factor2approximation", "christofidesandfactor2"]
        lagrangean_relaxation(exp_id, testdatafile, strategy, max_iterations, gap_threshold, epsilon, mi_option, epsilon_strategy, epsilon_decay_interval, epsilon_decay_multiplier, check_point)
    elseif strategy == "gurobi"
        lazy(testdatafile)
    elseif strategy == "lazyandchris"
        lower, upper = lagrangean_relaxation(exp_id, testdatafile, "christofides", max_iterations, gap_threshold, epsilon, mi_option, epsilon_strategy, epsilon_decay_interval, epsilon_decay_multiplier, check_point)
